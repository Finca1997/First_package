...
if(!empty($_POST['submit'])){
	...
	$sql = "SELECT * FROM `sheji`.`user` WHERE `username` = '$username' AND `password` = '$password'";
	$query = mysql_query($sql);
	$result = mysql_fetch_row($query);
	
	if(is_array($result)){
		$_SESSION['islogin'] = $result[1];
		echo "<script>alert('登陆成功');location.href='person.php';</script>";
	}else{
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-3">
                <h2>广油博友，欢迎回来</h2>
                <p>登陆后可按你的兴趣设置页面内容</p>
                <form  class="form1" name="form1" action="">
                    <input class="text_name" type="text" name="realname" id="realname" placeholder="微博/QQ/手机"><br>
                    <input class="text_password" type="password" name="pwd" placeholder="密码" id="pwd" onblur="checkPassword(this.value);">
                    <input  class="button_submit" type="button" value="登录" onclick="cheakrealName()">
                    <input type="checkbox" class="checkbox">
                    <span>自动登录</span>
                    <a class="wangji">忘记密码</a>
                    <a class="zhuce" href="newpeople2.html">立即注册</a>
                </form>
            </div>
